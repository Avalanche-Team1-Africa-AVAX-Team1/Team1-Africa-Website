    ...
    <div className="bg-gray-100 py-16">
      ...
      <div className="w-full px-8">
        <div className="flex items-center justify-center mt-8 gap-4">
          <button
            onClick={prevGame}
            className="w-12 h-12 bg-gray-800 hover:bg-gray-700 text-white rounded-full flex items-center justify-center transition-colors"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <div className="flex gap-3">
            {games.map((_, index) => (
              <button
                ...
                className={`w-4 h-4 rounded-full transition-all duration-300 ${index === currentGameIndex ? 'bg-gray-800 scale-125' : 'bg-gray-400 hover:bg-gray-600'
                  }`}
              />
            ))}
          </div>
